...
def assert_volume(compose, mount_dict):
    ...
    try:
        ...
        compose.podman.output(
            ["volume", "create", "--label", "io.podman.compose.project={}".format(proj_name), vol_name])
        out = compose.podman.output(["volume", "inspect", vol_name]).decode('utf-8')
...
def container_to_ulimit_args(cnt, podman_args):
    ...
    if ulimit is not None:
        ...
        # or a dictionary or list:
            ...
            ulimit = ["{}={}".format(ulimit_key, norm_ulimit(inner_value)) for ulimit_key, inner_value in
                      ulimit.items()]
            for i in ulimit:
...
def build_one(compose, args, cnt):
    ...
    if getattr(args, 'if_not_exists', None):
        try:
            img_id = compose.podman.output(['inspect', '-t', 'image', '-f', '{{.Id}}', cnt["image"]])
        except subprocess.CalledProcessError:
            img_id = None
        if img_id: return
...
def compose_ps(compose, args):
    ...
    if args.quiet == True:
        compose.podman.run(
            ["ps", "-a", "--format", "{{.ID}}", "--filter", f"label=io.podman.compose.project={proj_name}"])
    else:
